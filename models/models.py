...
class RNN_GNN(Model): 
    ...
    def forward(self, month_list,data_dict, device,h0 = None,train = True): 
        h0s =   self.RNN.init__hidd(device) if h0 == None else h0 
        hidden_states = h0s
        ...
        if self.upsample > 0 and train: 
            ...
            synth_index = []
        scores = self.decoder(torch.Tensor(last_h).to(device))
...
class RNN_only(Model):
    ...
    def forward(self, month_list,data_dict, device, h0, train = True): 
        h0s =   self.RNN.init__hidd(device) if h0 == None else h0 
        hidden_states = h0s
        for i,m in enumerate(month_list): 
            data = data_dict[m]
            labs = data.y
            hidden_states = self.RNN(torch.Tensor(data.x).to(device), (hidden_states
            )) 
            if i == 0:
                h0 = [get_hidden(hidden_states[0])]
        last_h = hidden_states[-1]
